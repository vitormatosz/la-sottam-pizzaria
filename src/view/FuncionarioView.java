...
public class FuncionarioView extends JPanel {
    ...
    private JLabel sectionTitle(String text) {
        MarqueeLabel label = new MarqueeLabel(text);
    ...
    private static class MarqueeLabel extends JLabel {
        ...
        MarqueeLabel(String text) {
            ...
            timer = new Timer(35, e -> {
                if (getTextWidth() > getWidth()) {
                    offset = (offset + 1) % (getTextWidth() + GAP);
                    repaint();
                } else if (offset != 0) {
                    offset = 0;
                    repaint();
        ...
        protected void paintComponent(Graphics graphics) {
            ...
            int x = textWidth > getWidth() ? -offset : 0;

            g2.setColor(getForeground());
            g2.setFont(getFont());
            g2.drawString(getText(), x, baseline);
            if (textWidth > getWidth()) {
                g2.drawString(getText(), x + textWidth + GAP, baseline);
            }
            g2.dispose();
        }
    }
...
>>>>>>> 3f57a16 (alinhando o formulario dos funcionarios)
}